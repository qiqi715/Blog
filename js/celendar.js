...
(function() {
    ...
    var oDeskTime = document.querySelector('.task-column .s-time');//桌面时间
    ...
    function getTime() {
        ...
        oDeskTime.innerHTML = toTwo(iHour) + ':' + toTwo(iMinute);
    }
...
(function(){
    ...
    var celendar = document.querySelector('.celendar');//日历
    var oDate = document.querySelector('.celendar .date');//日期-年月日
    ...
    document.addEventListener('click', function(ev) {
        if (ev.target.id != 'time') {
            oDateClick();
            document.querySelector('.celendar').style.display = 'none';
        }
    });

    celendar.addEventListener('click', function(ev) {
        ev.stopPropagation();
    });

    /*日期(年月日)鼠标点击 还原到当前月*/
    oDate.onclick = oDateClick;
    function oDateClick() {
        var date = new Date();
        if(iLevel != 0) {
            iActDate = date;;
            if (iLevel == 1) {
                ...
                iLevel = 1;
                toPreLevel(oHundredYear, oMonth, aMonthLi[0]);
            }
        } else {
